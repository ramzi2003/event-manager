...
function DashboardPage() {
  ...
  const [showLoader, setShowLoader] = useState(false); // Controls when the loader is displayed
  const user = useSelector((state) => state.auth.user);

  const fetchEvents = async () => {
    setLoading(true); // Start loading
    const loaderTimeout = setTimeout(() => setShowLoader(true), 500); // Delay loader by 500ms
  ...
  const fetchTasks = async (eventId) => {
    setLoading(true); // Start loading for tasks
    const loaderTimeout = setTimeout(() => setShowLoader(true), 500); // Delay loader by 500ms
